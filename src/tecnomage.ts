...
export class TecnomageController{
  ...
  async load(url='/assets/characters/tecnomage.gltf'){
    try{
      ...
      this.setState('IDLE');
      console.info('[M14-WEB-B2.2] Rigged Tecnomage loaded',gltf.animations.map(a=>a.name));
      return true;
    }catch(error){console.warn('[M14-WEB-B2.2] glTF unavailable; procedural fallback active.',error);return false}
  }

  update(dt:number,next:TecnomageState){this.setState(next);this.mixer?.update(dt);this.animateFallback(dt,next)}

  private setState(next:TecnomageState){if(next===this.state)return;const prev=this.state?this.actions.get(this.state):undefined,action=this.actions.get(next)??this.actions.get(next==='SPRINT'?'RUN':'IDLE');prev?.fadeOut(.16);action?.reset().fadeIn(.16).play();this.state=next}
